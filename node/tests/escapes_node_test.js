...
describe("Node.js escape tests (no sandbox — escapes expected to succeed)", () => {
    ...
    it("escape: webr::eval_js CAN read arbitrary host files (fs.readFileSync)", () => {
        ...
        assert.ok(hostname.length > 0, "Could not read /etc/hostname for test setup");

        const { code, stdout, stderr } = rw(
            `webr::eval_js('import("fs").then(m => m.readFileSync("/etc/hostname", "utf8"))', await = TRUE)\n`
        );

        assert.equal(code, 0, `Expected exit code 0 (escape should succeed), got ${code}\nstderr: ${stderr}`);
        assert.ok(
    ...
    it("escape: webr::eval_js CAN write to arbitrary host paths (fs.writeFileSync)", () => {
        const target = path.join(os.tmpdir(), `rw_escape_write_node_${Date.now()}.txt`);
