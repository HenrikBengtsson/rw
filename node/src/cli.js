...
export function parse_args(args) {
  ...
  for (const arg of args) {
    ...
    if (arg === "--help") {
      ...
      if (isNaN(options.timeout) || options.timeout < 0) {
        throw new Error(
          "Timeout must be a non-negative number of seconds: " + value,
        );
      }
      ...
      if (command.type === "install") {
        ...
        command.type === null && options.exprs.length === 0 && r_script === null
      ) {
        if (arg === "env") {
          command.type = "env";
        } else if (arg === "config") {
  ...
  if (!options.no_config) {
    ...
    if (options.r_libs_user === null && rwconfig["r-libs-user"]) {
      ...
      if (options.debug) {
        console.log(`r_libs_user=${options.r_libs_user} (from .rwconfig)`);
      }
    }
    if (options.bastion_host === null && rwconfig["bastion"]) {
      options.bastion_host = rwconfig["bastion"];
      if (options.debug) {
        console.log(`bastion_host=${options.bastion_host} (from .rwconfig)`);
      }
    }
    if (
      options.prologue_exprs.length === 0 && r_prologue_script === null &&
      rwconfig["prologue-expr"]
    ) {
      options.prologue_exprs.push(rwconfig["prologue-expr"]);
      if (options.debug) {
        console.log(
          `prologue_expr=${rwconfig["prologue-expr"]} (from .rwconfig)`,
        );
      }
    }
    if (
      options.epilogue_exprs.length === 0 && r_epilogue_script === null &&
      rwconfig["epilogue-expr"]
    ) {
      options.epilogue_exprs.push(rwconfig["epilogue-expr"]);
      if (options.debug) {
        console.log(
          `epilogue_expr=${rwconfig["epilogue-expr"]} (from .rwconfig)`,
        );
      }
    }
  }
  ...
  if (options.verbose && !options.no_config) {
    ...
    if (
      fs.existsSync(RWCONFIG_PATH) &&
      path.resolve(RWCONFIG_PATH) !== path.resolve(USER_RWCONFIG_PATH)
    ) {
  ...
  if (r_prologue_script !== null) {
    r_prologue_script = normalize_path(r_prologue_script, "host file");
  }
  if (r_epilogue_script !== null) {
    r_epilogue_script = normalize_path(r_epilogue_script, "host file");
  }
  if (options.r_libs_user !== null) {
    options.r_libs_user = normalize_path(options.r_libs_user, "host directory");
  }
  if (options.bastion_host !== null) {
    options.bastion_host = normalize_path(
      options.bastion_host,
      "host directory",
    );
  }
  for (const bind of options.binds) {
    bind.host = normalize_path(bind.host, "host directory");
  }
  ...
  if (options.prologue_exprs.length > 0 && r_prologue_script !== null) {
    ...
    options.prologue_exprs = read_code(
      r_prologue_script,
      "prologue",
      options.debug,
    );
  ...
  if (options.epilogue_exprs.length > 0 && r_epilogue_script !== null) {
    ...
    options.epilogue_exprs = read_code(
      r_epilogue_script,
      "epilogue",
      options.debug,
    );
    ...
    if (!options.no_config) {
      ...
      if (rwconfig_opt) {
        ...
        if (eq !== -1 && rwconfig_opt.slice(0, eq).trim() === "shims") {
          options.shims = rwconfig_opt.slice(eq + 1).split(",").filter((str) =>
            str !== ""
          );
        }
      }
...
async function main() {
  ...
  if (command.type === "env") {
    ...
    if (command.action === "list") {
      ...
      console.error(
        "ERROR: Unknown env action. Use 'rw env list' or 'rw env get <field>'",
      );
      process.exit(1);
    }
    let exit_code;
  ...
  if (command.type === "config") {
    ...
    if (command.action === "list") {
      ...
      if (entries.length === 0) {
        const where = command.scope !== null
          ? scope_label
          : "~/.rwconfig or ./.rwconfig";
        console.log(`(no settings in ${where})`);
      } else {
        for (const [k, v] of entries) {
          console.log(`${k}=${v}  # ${provenance[k]}`);
        }
      }
    } else if (command.action === "get") {
      if (command.field === null) {
      ...
      if (!command.field || command.value === null) {
        console.error(
          "ERROR: 'rw config set' requires a field name and a value",
        );
        process.exit(1);
      }
      ...
      console.error(
        "ERROR: Unknown config action. Use 'rw config list', 'rw config get <field>', 'rw config set <field> <value>', or 'rw config unset <field>'",
      );
      process.exit(1);
    }
    process.exit(0);
  ...
  if (command.type === "build") {
    if (!command.build_docker) {
      console.error(
        "ERROR: 'rw build' requires --docker (host-native build not yet supported)",
      );
      process.exit(1);
    }
    const pkg_path = normalize_path(
      command.build_path ?? ".",
      "package directory",
    );
    const out_path = process.cwd();
    if (options.verbose) {
      console.error(`Building package from '${pkg_path}' using Docker`);
    }
  ...
  if (command.type === "install") {
    if (command.install_packages.length === 0) {
      console.error(
        "ERROR: 'rw install' requires at least one package name or directory",
      );
      process.exit(1);
    }
    ...
    if (!options.r_libs_user) {
      console.error(
        "ERROR: 'rw install' with --persistent requires --r-libs-user=<dir> or r-libs-user in .rwconfig",
      );
      process.exit(1);
    }
      ...
      for (const pkg_dir of command.install_packages) {
        ...
        if (options.verbose) {
          for (const f of built) {
            console.error(`Built tarball '${path.join(out_path, f)}'`);
          }
        }
        tarballs.push(...built);
      }
  ...
  if (command.type === "uninstall") {
    ...
    if (!options.r_libs_user) {
      console.error(
        "ERROR: 'rw uninstall' with --persistent requires --r-libs-user=<dir> or r-libs-user in .rwconfig",
      );
      process.exit(1);
    }
    ...
    const pkgs_r = `c(${
      command.uninstall_packages.map((p) => JSON.stringify(p)).join(", ")
    })`;
